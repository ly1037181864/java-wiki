...
会出现脏读的情况。另外二级缓存在更新操作时会清除掉二级缓存和一级缓存，但是如果线程在执行commit前被挂起，而另一个线程执行查询操作时，则会将从数据库拿
到的记录缓存到一级缓存中去，操作完后，被挂起的线程获得执行权限，就会将上一波线程放入到二级缓存中的数据，同步到一级缓存中去，但实际上这个数据并不是最新
的，也即这个时候是会出现脏读的情况。
